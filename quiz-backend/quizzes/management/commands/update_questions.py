...
class Command(BaseCommand):
    help = 'Update quiz questions with programming content'

    def handle(self, *args, **options):
        self.stdout.write("Updating questions with better programming content...")
        
        # Get the Demo Quiz
        demo_quiz = Quiz.objects.get(id=1)
        self.stdout.write(f"Updating questions for: {demo_quiz.title}")
        ...
        new_questions = [
            ...
            },
            {
                ...
                'options': [
                    {'text': 'let variableName;', 'is_correct': True},
                    {'text': 'variable variableName;', 'is_correct': False},
                    {'text': 'v variableName;', 'is_correct': False},
        ...
        for i, question_data in enumerate(new_questions, 1):
            ...
            self.stdout.write(f"Created question {i}: {question_data['text']}")
        
        self.stdout.write(
            self.style.SUCCESS(f"Successfully updated {len(new_questions)} questions!")
        ) 